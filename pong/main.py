...
while run:
    if not start:
        ...
        opponent_target_y = ball.rect.y - OPPONENT.height // 2
        rnd: float = random.random()
        mistake_chance: float = random.random() / DIFFICULTY

        if rnd < mistake_chance:
            mistake_offset = random.randint(-MISTAKE_OFFSET, MISTAKE_OFFSET)
            opponent_target_y += mistake_offset
        else:
            if OPPONENT.y < opponent_target_y:
                OPPONENT.y += min(1, opponent_target_y - OPPONENT.y) 
            elif OPPONENT.y > opponent_target_y:
                OPPONENT.y -= min(1, OPPONENT.y - opponent_target_y)
